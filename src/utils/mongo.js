...
export async function start() {
  if (db) return db; // RETURN the existing db if it's already there
  try {
    await client.connect();
    db = client.db("textqanda");
    console.log("Successfully connected to MongoDB");
    return db; // RETURN the newly created db instance
  } catch (error) {
    console.error(`[CONNECTION ERROR]:`, error.message);
    throw error; // Throw so the API handler knows it failed
  }
}
...
export async function createData(collectionName, data) {
  ...
  try {
    ...
    return result.insertedId; // Returns the new MongoDB ObjectId // EventId parameter in frontend files
  } catch (error) {
